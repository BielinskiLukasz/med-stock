...
**Domain:** Local-first PWA medicine inventory app
...
```bash
# Core dependencies
npm install react react-dom zustand dexie zod
...
npm install -D @types/react @types/react-dom @types/node @types/papaparse
...
**Pitfall:** Manual migration functions with missing version checks cause data corruption when upgrading app versions.
**Prevention:** Use Dexie.js which handles schema versioning automatically. Do not hand-write IndexedDB upgrade functions.

### Over-Engineering State Management
**Pitfall:** Using Redux or Jotai when Zustand suffices leads to unnecessary boilerplate and slower development.
**Prevention:** Start with Zustand for this project; only upgrade to Redux if you hit a genuine scalability wall.

### React Hook Form & Uncontrolled Inputs
**Pitfall:** Trying to make React Hook Form fields controlled (like form value as state) defeats performance benefits.
**Prevention:** Keep forms uncontrolled; use React Hook Form's watch() only for specific fields that need reactive rendering.

### CSV Import Without Validation
**Pitfall:** Importing malformed CSV or invalid data (e.g., expiration dates) corrupts the IndexedDB.
**Prevention:** Always validate imported CSV rows with Zod schema before committing to Dexie.js.

### Service Worker Caching Too Aggressively
**Pitfall:** Old app shell cached in service worker; users cannot get app updates.
**Prevention:** Use vite-plugin-pwa's default 'autoUpdate' mode and Workbox's skipWaiting + clientsClaim to force service worker activation.
...
---
*Stack research for: MedStock — Local-first PWA medicine inventory app*
*Researched: 2026-06-29*
*Confidence: HIGH across all categories*
